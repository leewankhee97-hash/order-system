                ...
                agents.map((agent) => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full rounded-[22px] border border-[#e8d8c8] bg-white p-4 text-left shadow-sm transition hover:border-[#c8a487]"
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <div className="font-semibold text-[#5f3d2e]">
                            {agent.code || agent.agent_name}
                          </div>
                          <div className="mt-1 text-sm text-[#8b6a57]">
                      ...
                      <div className="mt-3 space-y-1 text-xs text-[#8b6a57]">
                        <div>/order/{currentSlug}</div>
                        <div>
                          Prefix: {agent.order_prefix || '-'} | Counter: {agent.order_counter || 1}
                        </div>
                        <div>{agent.is_active ? 'Active' : 'Inactive'}</div>
                      </div>
                    </button>
                  )
                })
            ...
            <div className="mt-5 rounded-2xl bg-[#f8f1ea] p-4 text-sm text-[#7b5a49]">
              <div>
                ...
                <span className="ml-2 font-medium text-[#5f3d2e]">
                  /order/{makeSlug(form.slug || form.code || '')}
                </span>
              </div>
              <div className="mt-2">
                自取预览：
