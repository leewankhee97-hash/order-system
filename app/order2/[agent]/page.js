...
function getBundleProductsByRole(
  bundle,
  allProducts,
  role,
  fallbackToBrand = false,
) {
  if (!bundle) return [];
...
function getSummaryVariantName(item) {
  ...
  if (eqText(cleanName, groupName))
    return normalizeText(item?.name) || cleanName || "-";
...
export default function Page() {
    ...
    try {
      ...
      for (let retry = 0; retry < 3 && !bundlesData; retry++) {
        const { data: b, error: bundlesError } = await supabase
          .from("bundle_rules")
          .select(
            `*, bundle_rule_products(id, bundle_rule_id, product_id, role, qty_required, is_gift, choose_required, random_only)`,
          )
          .eq("is_active", true)
  ...
  function getAgentLevel() {
    const raw = String(agentInfo?.level ?? "")
      .trim()
      .toLowerCase();
  ...
  function addDraftToCart(product) {
    ...
    if (product.is_muar_only && hasNormalItem) {
      showMuarNotice(
        "此产品是在 MUAR 出货，不可与其他产品一起下单，请分开下单",
      );
      return;
    }
  ...
  function addBundleToCart() {
    ...
    const bundleType = normalizeText(
      selectedBundle.bundle_type || "buy_x_get_y",
    );
    const giftMode = normalizeText(selectedBundle.gift_mode || "none");
    const giftChooseMode = normalizeText(
      selectedBundle.gift_choose_mode || "none",
    );
    ...
    if (bundleType === "fixed_combo") {
      ...
      if (bundleBuyQty > 0 && bundleCount % bundleBuyQty !== 0) {
        setError(
          `这个 Bundle 每组需要购买 ${bundleBuyQty} 个，目前还差 ${bundleBuyQty - (bundleCount % bundleBuyQty)} 个`,
        );
        return;
      }
      ...
      if (requiredGiftPodQty > 0) {
        giftItems = buildSelectionItems(
          bundleGiftSelect,
          bundleGiftPodProducts,
          "gift_pod",
        );
        const selectedGiftQty = giftItems.reduce(
        ...
        if (selectedGiftQty !== requiredGiftPodQty) {
          setError(
            `这个 Bundle 需要选择 ${requiredGiftPodQty} 个赠品烟弹，目前已选 ${selectedGiftQty} 个`,
          );
          return;
        }
      }
  ...
  const typeOptions = useMemo(() => {
    ...
    const all = [
      ...new Set(products.map((p) => getProductType(p)).filter(Boolean)),
    ];
  ...
  const brandOptions = useMemo(() => {
    ...
    return [
      ...new Set(
        products
          .filter((p) => {
            if (selectedType && getProductType(p) !== selectedType)
              return false;
 
            if (!q) return true;
 
            const target = [p.brand, p.series, p.name, cleanProductName(p)]
              .filter(Boolean)
              .join(" ")
              .toLowerCase();
  ...
  const bundleGiftDeviceProducts = useMemo(() => {
    return getBundleProductsByRole(
      selectedBundle,
      products,
      "gift_device",
      false,
  ...
  const selectedBundleProduct = useMemo(() => {
    ...
    return (
      bundleProducts.find(
        (p) => cleanProductName(p) === selectedBundleFlavor,
      ) || null
    );
  ...
  function setBundleGiftQty(pid, qty) {
    const targetProduct =
      bundleGiftPodProducts.find((p) => String(p.id) === String(pid)) ||
  ...
  function setComboPodQty(pid, qty) {
    const targetProduct = bundleComboPodProducts.find(
      (p) => String(p.id) === String(pid),
    );
    const maxStock = Number(targetProduct?.stock || 0);
    let next = Number(qty || 0);
  ...
  function setComboDeviceQty(pid, qty) {
    const targetProduct = bundleComboDeviceProducts.find(
      (p) => String(p.id) === String(pid),
    );
    const maxStock = Number(targetProduct?.stock || 0);
    let next = Number(qty || 0);
  ...
  function buildCopiedSummary(oid) {
    ...
    cart
      ...
      .forEach((item) => {
        ...
        if (
          ...
          (item.bundle_random_gifts || []).length > 0
        ) {
          lines.push("");
          lines.push(`赠品：`);
    ...
    lines.push(
      `运费：${
        shippingFee === "ASK" ? "称重后通知" : `RM${money(shippingFee)}`
      }`,
    );
        ...
        <div className="mb-6 overflow-hidden rounded-[30px] border border-[#eadacb] bg-white/75 shadow-[0_20px_60px_rgba(121,88,63,0.12)] backdrop-blur">
          ...
          <div className="grid gap-3 p-5 md:grid-cols-4">
            ...
            <div className="rounded-3xl border border-[#eadccf] bg-[#fbf6f1] p-4">
              <div className="text-xs font-bold uppercase tracking-widest text-[#8a6247]">
                CART ITEMS
              </div>
              <div className="mt-2 text-3xl font-black text-[#7b5740]">
                {cartQty}
              </div>
            </div>
          </div>
            ...
            <section className="rounded-[30px] border border-[#eadacb] bg-white/80 p-5 shadow-[0_15px_45px_rgba(121,88,63,0.10)] backdrop-blur">
              <div className="mb-4 flex flex-wrap items-end justify-between gap-4">
                <div>
                  ...
                  <h2 className="mt-2 text-xl font-black text-[#5f4432]">
                    Products
                  </h2>
                </div>
                  ...
                  return (
                    ...
                    >
                      <div className="mb-2">
                        ...
                        <div
                          className={`mt-3 inline-flex shrink-0 rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide ${stockInfo.badge}`}
                        >
                          {stockInfo.text}
                        </div>
                      </div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {bundleFlavorOptions.map((flavor) => (
                                <FilterButton
                                  ...
                                  onClick={() =>
                                    setSelectedBundleFlavor(flavor)
                                  }
                                >
                                  {flavor}
                                </FilterButton>
                            ...
                            >
                              ...
                              <div className="mt-4 grid grid-cols-2 gap-3">
                                ...
                                <div className="rounded-3xl border border-[#eadacb] bg-[#fffaf6] p-3">
                                  ...
                                  <div className="mt-2 text-xl font-black text-[#7b5740]">
                                    {Number(
                                      bundleSelect[selectedBundleProduct.id] ||
                                        0,
                                    )}
                                  </div>
                                </div>
                              ...
                              <div className="mt-4 rounded-[24px] border border-[#eadacb] bg-[#fffaf6] p-3">
                                ...
                                <div className="mt-2 text-center text-xs text-[#8b7260]">
                                  {Number(
                                    bundleSelect[selectedBundleProduct.id] || 0,
                                  ) > 0
                              ...
                              >
                                ...
                                <div className="flex items-center gap-2">
                                  <button
                                    type="button"
                                    onClick={() =>
                                      changeBundleGiftQty(p.id, -1, p.stock)
                                    }
                                    className="h-9 w-9 rounded-full border border-[#eadacb] bg-white text-[#6c513d] hover:bg-[#f8efe6]"
                                  >
                                    -
                                  ...
                                  <input
                                    ...
                                    onChange={(e) =>
                                      setBundleGiftQty(p.id, e.target.value)
                                    }
                                    className="h-9 flex-1 rounded-full border border-[#eadacb] bg-white px-3 text-center text-sm font-bold text-[#5f4432] outline-none"
                                  />
                      ...
                      >
                          ...
                          <div className="mt-3 rounded-3xl border border-[#d8ead9] bg-white/80 p-3">
                            ...
                            <div className="mt-2 space-y-1 text-sm text-[#446148]">
                              {(item.bundle_items || []).map((bi, idx) => (
                                <div
                                  key={`${item.id}-${bi.product_id}-main-${idx}`}
                                >
                        ...
                        {(item.bundle_combo_items || []).length > 0 ? (
                          <div className="mt-3 rounded-3xl border border-[#d8ead9] bg-white/80 p-3">
                            ...
                            <div className="mt-2 space-y-1 text-sm text-[#446148]">
                              {(item.bundle_combo_items || []).map(
                                (bi, idx) => (
                                  <div
                                    key={`${item.id}-${bi.product_id}-combo-${idx}`}
                                  >
                                    {bi.role === "combo_device"
                                      ? "烟枪"
                                      : "烟弹"}
                                    ：{bi.product_name} - {bi.qty}
                                  </div>
                          ...
                          <div className="mt-3 rounded-3xl border border-[#f5e1b8] bg-[#fffaf0] p-3">
                            ...
                            <div className="mt-2 space-y-1 text-sm text-[#6c552f]">
                              {(item.bundle_gift_items || []).map((bi, idx) => (
                                <div
                                  key={`${item.id}-${bi.product_id}-gift-${idx}`}
                                >
                                  {bi.product_name} - {bi.qty}
                                </div>
                              ))}
 
                              {(item.bundle_random_gifts || []).map(
                                (gift, idx) => (
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            ...
                            <div className="mt-2 text-sm text-[#7b5740]">
                              RM {money(item.price)}
                            </div>
                          </div>
            ...
            <section className="rounded-[30px] border border-[#eadacb] bg-white/80 p-5 shadow-[0_15px_45px_rgba(121,88,63,0.10)] backdrop-blur">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between rounded-3xl border border-[#d8c2aa] bg-[linear-gradient(90deg,#f6eadf_0%,#edd8c4_100%)] px-4 py-4">
                  ...
                  <span className="text-2xl font-black text-[#6a4a34]">
                    RM {money(total)}
                  </span>
                </div>
